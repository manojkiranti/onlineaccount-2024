...
function requiredIfCoapplicantPR<T extends yup.AnySchema>(
  schemaType: T,
  message: string,
): T {
  return schemaType.when(['coapplicantCitizenship '], {
    is: (citizenship: string) =>
      citizenship === 'australian_pr_visa' ||
      citizenship === 'australian_tr_visa',
    then: (schema: T) => schema.required(message),
...
export const incomeDetailSchema = yup.object().shape({
  ...
  typeOfAllowanceIncome: yup
    .array()
    .of(yup.string())
    .when('receiveAllowanceIncome', {
      is: true,
      then: (schema) =>
        schema.required('Please specify the types of allowance income.'),
      otherwise: (schema) => schema.notRequired(),
    }),
  ...
  coapplicantCreditCardCurrency: yup.string().when('coapplicantExists', {
    is: true,
    then: (schema) =>
      schema.required('Please verify your credit card currency'),
    otherwise: (schema) => schema.notRequired(),
  }),
  coapplicantCreditCardLimit: yup.number().when('coapplicantExists', {
    is: true,
    then: (schema) => schema.required('Credit card limit is required'),
    otherwise: (schema) => schema.notRequired(),
  }),
  ...
  coapplicantBaseSalaryFrequency: requiredIfCoapplicantIncomeSource(
    yup.string(),
    ['employee'],
    'Please select your base salary frequency.',
  ),
  ...
  coapplicantCommissionFrequency: yup
    .string()
    .when('coapplicantReceiveCommissionIncome', {
      is: true,
      then: (schema) =>
  ...
  coapplicantVisaSubClass: requiredIfCoapplicantPR(
    yup.string(),
    'Visa sub class is required.',
  ),
  coapplicantVisaExpiryDate: requiredIfCoapplicantPR(
    yup.string(),
