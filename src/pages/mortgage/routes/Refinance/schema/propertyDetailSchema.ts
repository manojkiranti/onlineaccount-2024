...
for (let i = 1; i <= MAX_PROPERTIES; i++) {
  ...
  propertyFields[`ausPropertyAmountRequired${i}`] = yup
    .number()
    .when(`ausPropertyPurpose${i}`, {
      is: (ausPropertyUseValue: any) =>
        ausPropertyUseValue === 'refinance_and_cash_out' ||
        ausPropertyUseValue === 'cash_out_only',
      then: (schema) => schema.required(`Please enter amount required ${i}`),
      otherwise: (schema) => schema.notRequired(),
    });

  propertyFields[`ausPropertyAmountPurpose${i}`] = yup
    .string()
    .when(`ausPropertyPurpose${i}`, {
      is: (ausPropertyUseValue: any) =>
        ausPropertyUseValue === 'refinance_and_cash_out' ||
        ausPropertyUseValue === 'cash_out_only',
      then: (schema) =>
        schema.required(`Please specify the property purpose for ${i}`),
      otherwise: (schema) => schema.notRequired(),
    });
}
...
for (let i = 1; i <= MAX_PROPERTIES; i++) {
  ...
  overseasPropertyFields[`overseasPropertyRent${i}`] = yup
    ...
    .when(`overseasPropertyUse${i}`, {
      is: 'investment_property',
      then: (schema) => schema.required(`Please enter property rent ${i}`),
      otherwise: (schema) => schema.notRequired(),
    });
}
...
//     propertyLoaction: yup.string().required("Please select property location"),
//     maxDepositDownpayment: yup.number().required("Please enter your maximum deposit down payment amount"),

//     ausProperty: yup.number().required("Please select your Australia property size"),
//     ...propertyFields,

//     useExistingAusProperty: yup.string().required("Please select your preferred option"),

//     overseasProperty: yup.number().required("Please select your overseas property size"),
...
export const propertyDetailLabelMapper = {
  ...
  propertyLoaction: 'Which state will this property be located?',
  ...
  ausMortgageDetails: 'Mortgage details',
  ...
  overseasMortgageDetails: 'Mortgage details',
