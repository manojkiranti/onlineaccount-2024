...
import { IncomeDetailPayload } from '@/pages/mortgage/types/Refinance';
import {
...
} from '../../Purchase/constant/optionList';
...
const IncomeDetail = () => {
  ...
  const [postRefinanceRequest, { isLoading: postRefinanceRequestLoading }] =
    usePostRefinanceRequestMutation();
  ...
  useEffect(() => {
    if (id && incomeDetailData) {
      ...
      if (incomeDetailData?.receiveCommissionIncome) {
        ...
        setValue('commissionFrequency', incomeDetailData?.commissionFrequency);
      ...
      if (
        incomeDetailData?.incomeSource ===
          'self_employed_trader_or_contractor' ||
        incomeDetailData?.incomeSource === 'self_employed_company'
      ) {
      ...
      if (incomeDetailData?.coapplicantExists) {
        ...
        if (incomeDetailData?.coapplicantIncomeSource === 'employee') {
          ...
          setValue(
            'coapplicantBaseSalaryFrequency',
            incomeDetailData?.coapplicantBaseSalaryFrequency,
          );

          if (incomeDetailData?.coapplicantReceiveAllowanceIncome) {
            setValue(
          ...
          if (incomeDetailData?.coapplicantReceiveCommissionIncome) {
            setValue(
  ...
  const onSubmit = (data: IncomeDetailFormType) => {
    ...
    postRefinanceRequest(payload)
      ...
      .then((res) => {
        ...
        navigate(`/mortgage/refinance`, { replace: true });
      })
      .catch((err) => {
        displayError(err);
      });
  ...
  const handleBackClick = () => {
    navigate(`${refinanceFormRoutes['property_detail']}/${id}`);
  };
  ...
  return (
    <StepperFormLayout
      ...
      isSubmitLoading={postRefinanceRequestLoading}
    >
      <Container width="md">
        <Row>
          <Col xs={24}>
                ...
                <Col xs={24}>
                  <div className="form-section-header form-control">
                    <FormFieldHeading
                      total={total}
                      current={4}
                      title="Refinancing this property with someone else?"
                      subTitle="We need these to assess your tax situation to give you the best tax outcome."
                    />
                  </div>
                  <Row gutter={30}>
                    <Col xs={24}>
                      <ListOptionsField
                        ...
                        label="Are you refinancing this property with someone else?"
                        control={control}
                    ...
                    <Row gutter={30}>
                      ...
                      <Col xs={24}>
                        <SelectField
                          ...
                          error={errors?.coapplicantCitizenship?.message ?? ''}
                          size="large"
                        />
                      </Col>
                      {(watchCoapplicantCitizenship === 'australian_pr_visa' ||
                      ...
                      {watchCoapplicantIncomeSource === 'employee' && (
                        <CoapplicantEmployeeField
                          ...
                          watchAllowanceIncome={watchCoapplicantAllowanceIncome}
                          watchCommissionIncome={
                            watchCoapplicantCommissionIncome
                          }
                          watchAnnualCashBonus={watchCoapplicantAnnualCashBonus}
                      ...
                      <Col xs={24} md={12}>
                        <SelectField
                          ...
                          placeholder="Select currency type"
                          error={
                            errors?.coapplicantCreditCardCurrency?.message ?? ''
                          }
                        />
                      </Col>
                      <Col xs={24} md={12}>
